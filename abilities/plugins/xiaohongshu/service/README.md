...
- `src/browser/`：长期 Browser 与按账号隔离的 BrowserContext；普通状态检查和 MCP 操作复用各自 context，只有登录会话创建可见 context。
...
MCP 的账号级工具接受可选 `account_id` 参数。未提供时使用当前活跃账号；提供后直接路由到该账号的隔离 BrowserContext，因此多个账号可以同时执行请求，不需要停止服务或手动切换。
