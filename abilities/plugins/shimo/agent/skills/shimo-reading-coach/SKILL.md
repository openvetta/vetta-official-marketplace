...
description: >-
  ...
  and keep uncertainty visible. Use Shimo's selected reading model and save the reading record
  automatically. Do not invent a separate chat history, duplicate the book, or claim that an answer
...
category in the context supplied by Shimo. Send the request to the reading model selected in Shimo.
Do not build a separate message list. The answer is saved by default;
